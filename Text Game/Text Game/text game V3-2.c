...
03-Apr-2023:	Phase 3-4 of project (final demo!).
				Fixed parameter mismatch error in
				a usage of UseItem().
				Created WaitForUser().
				Replaced most usages of Sleep() with
				WaitForUser().
				Created Intro() area.
************************************************/
...
//--- ROOMS ---
void Intro(USER* user, char* dataFile);
...
/************************** GetInput() ***************************
Purpose:	checks for the user's input before continuing program
...
******************************************************************/
void WaitForUser() {
	//bit of a hack solution but gets rid of leftover newline
	//from scanf_s
	char ch;
	ch = getchar();
...
USER CreateUser(INVENTORY inv) {
	USER user;
	printf("Please enter a username (cannot contain any spaces): ");
	scanf_s("%s", user.username, MAX_LENGTH);

	printf("Please enter the password (cannot contain any spaces): ");
...
//--- ROOMS ---
void Intro(USER* user, char* dataFile) {
	printf("You seem to have awoken in a bed that is not your own, in a room you've never\n");
	printf("seen before. How did you get here? Well, that's not important right now.\n");
	printf("The more important matter is how to get out of here. How difficult can that be?\n");
	printf("\n... Welcome to Don't Go Outside! Your goal is simple. Leave the strange\n");
	printf("house you have found yourself within. No matter what.\n");
